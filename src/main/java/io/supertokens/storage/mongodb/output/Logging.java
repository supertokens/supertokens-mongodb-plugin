...
package io.supertokens.storage.mongodb.output;
...
public class Logging extends ResourceDistributor.SingletonResource {
    ...
    private static final String RESOURCE_ID = "io.supertokens.storage.mongodb.output.Logging";
    private final Logger infoLogger;
    private final Logger errorLogger;

    private Logging(Start start, String infoLogPath, String errorLogPath) {
        this.infoLogger = infoLogPath.equals("null")
                ? createLoggerForConsole(start, "io.supertokens.storage.mongodb.Info." + start.getProcessId())
                : createLoggerForFile(start, infoLogPath,
                        "io.supertokens.storage.mongodb.Info." + start.getProcessId());
        this.errorLogger = errorLogPath.equals("null")
                ? createLoggerForConsole(start, "io.supertokens.storage.mongodb.Error." + start.getProcessId())
                : createLoggerForFile(start, errorLogPath,
                        "io.supertokens.storage.mongodb.Error." + start.getProcessId());
    }
