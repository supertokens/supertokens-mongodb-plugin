...
public class JWTSigningQueries {
    ...
    public static boolean setJWTSigningKeyInfoIfNoKeyForAlgorithmExists_Transaction(Start start, JWTSigningKeyInfo keyInfo) throws StorageQueryException {
        MongoDatabase client = ConnectionPool.getClientConnectedToDatabase(start);
        MongoCollection collection = client.getCollection(Config.getConfig(start).getJWTSigningKeysCollection());

        Document toInsertIfNoneFound = new Document(
                "$setOnInsert",
                new Document("_id", keyInfo.keyId)
                    .append("key_string", keyInfo.keyString)
                    .append("created_at", keyInfo.createdAtTime)
                    .append("algorithm", keyInfo.algorithm)
                );
         ...
         * This means that when setting Key2 when Key1 (with a matching algorithm) already exists in storage, the query will
         * return Key1. So after the query we need to compare the returned document with the one we were trying to set to determine
         * if the set query succeeded.
         */
        Document result = (Document) collection.findOneAndUpdate(
                Filters.eq("algorithm", keyInfo.algorithm),
                toInsertIfNoneFound,
                options
        );

        /*
         * Because we use findOneAndUpdate we cannot solely rely on key id to determine if a document was inserted or not
         ...
         * different key string than the one in storage. To resolve this we will first convert the Document to the key class
